...
describe("buildInvoicePdf", () => {
  it("produces a non-empty PDF buffer with %PDF header", async () => {
    ...
    const pdf = await buildInvoicePdf({
      ...
      exemption_note: null,
      situation_label: "F.P. FOURNITURE ET POSE",
      total_supply_ht: 6000,
      total_installation_ht: 4000,
      retention_rate: 0.05,
      retention_amount: 500,
      retention_status: "HELD",
      retention_due_date: "2027-09-14",
      stamp_amount: 0,
      net_payable: 9500,
      lines: [
        {
          ...
          quantity: 2,
          unit_price_ht: 5000,
          supply_unit_price_ht: 3000,
          installation_unit_price_ht: 2000,
  ...
  it("marks cancelled invoices without inventing issuer legal IDs", async () => {
    ...
    const pdf = await buildInvoicePdf({
      ...
      exemption_note: null,
      situation_label: "F. FOURNITURE",
      total_supply_ht: 7800,
      total_installation_ht: 0,
      retention_rate: 0,
      retention_amount: 0,
